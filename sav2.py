...
def create_filter(type, freq_hz, cut_off_freq, filter_order):
    ...
    Wn = cut_off_freq * 2 / freq_hz    # Normalized frequency (6 / 62.5) = 0.096

    # Catch when frequency too low
    if (Wn >= 1.0):
        raise Exception('Cut-off frequency is above Nyquist (1/2 sample rate ('+str(freq_hz/2)+')) Choose lower cutoff frequency.')

    return signal.butter(filter_order, Wn=Wn, btype=type)
...
def generate_features(implementation_version, draw_graphs, raw_data, axes, sampling_freq, scale_axes,
                      ...
                      spectral_peaks_threshold, spectral_power_edges, do_log):
    ...
    if (filter_type == 'low' or filter_type == 'high') and filter_order > 0:
        butter_b, butter_a = create_filter(filter_type, sampling_freq, filter_cutoff, filter_order)

        # Design check (Python only step)
        if not filter_is_stable(filter_order, filter_cutoff, sampling_freq):
            raise Exception('Filter created is not stable. Move cutoff away from 0 or Nyquist')
    ...
    for ax in range(0, len(axes)):
        ...
        if butter_b is not None:
            fx = signal.lfilter(butter_b,butter_a,fx)
        ...
        if (implementation_version == 1):
            ...
            freqs, spec_powers = signal.periodogram(fx, sampling_freq, nfft=fft_length)
            freq_spacing = freqs[1] - freqs[0]
            if do_log:
                spec_powers = np.log(zero_handling(spec_powers))
            ...
            # Also, nyquist bin is generally ignored
    ...
    if draw_graphs:
        if butter_b is not None:
            w, h = signal.freqz(butter_b, butter_a, fs=sampling_freq)

            graphs.append({
                'name': 'Filter response (dB)',
                'X': [20 * np.log10(abs(h)).tolist()],
                'y': w.tolist()
            })
    ...
    if (implementation_version == 1):
        ...
        for k in range(0, len(spectral_power_edges) - 1):
            labels.append('Spectral Power ' + str(spectral_power_edges[k]) + ' - ' + str(spectral_power_edges[k + 1]) + ' Hz')
...
def filter_is_stable(filter_order, filter_cutoff, sampling_freq):
    ...
    a = np.tan(M_PI * filter_cutoff / sampling_freq)
    a2 = pow(a, 2)
    A = np.empty(n_steps)
    d1 = np.empty(n_steps)
    d2 = np.empty(n_steps)
    ...
    total = np.abs(np.append(A,[d1,d2]))
    return np.max(total) / np.min(total) < 1e6
